...
var krAudio = {
	...
	init:function(){//播放器初始化
		...
		if(playlist == "fineListTab" || playlist == "hostListTab" || playlist == "searchList")
		{
			this.allItem = $("#boxSongList").children('.list-item').length; //播放列表的总数
			this.time(); //正在播放和播放结束
	...
	seturl:function(){
		...
		if(playlist == "fineListTab" || playlist == "hostListTab" || playlist == "searchList")
		{
			var currentObject = $("#boxSongList .list-item").eq(this.Currentplay-1); //获取当前播放对象
	...
	play:function(){
		...
		if(!search && this.changeURL){ //搜索标志位是false和播放地址改变才会进入
			if(playlist == "fineListTab" || playlist == "hostListTab" || playlist == "searchList")
			{
				 currentObject = $("#boxSongList .list-item").eq(this.Currentplay-1); //获取当前播放对象
				 currentObject.addClass("list-playing").siblings().removeClass("list-playing");  // 添加正在播放样式
			}else
			{
				 currentObject = $("#main-list .list-item").eq(this.Currentplay-1); //获取当前播放对象
				 currentObject.addClass("list-playing").siblings().removeClass("list-playing");  // 添加正在播放样式
			}
			...
			var songName = currentObject.find(".music-name").first().text()
			$("#music-cover").attr("src",pic);
			$("#songNames").text(songName);
			//背景切换图片
			blurImages(pic);
			//获取音乐标题
